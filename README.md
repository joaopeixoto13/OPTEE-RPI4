...
This Makefile is responsible to not only to compile the ARM Trusted Firmware and the Trusted OS, but also to **concatenate** this two binaries into one binary to be loaded in the memory. However, to ensure that bl32 binary will be placed in the FIP address, the bl31 binary must be fixed size of 128k bytes. Therefore, the Makefile is also responsible to truncate the bl31 binary to this size.
...
First, copy the [config.txt](https://github.com/joaopeixoto13/OPTEE-RPI4/blob/main/config.txt) file and past them into the `buildroot/output/images/rpi-firmware` directory.
...
Change the `cmdline.txt` located in `buildroot/output/images/rpi-firmware` directory to:
```
...
```
cd OPTEE-RPI4
sudo dd if=buildroot/output/images/sdcard.img of=/dev/mmcblk0 
```
...
Make sure to also copy the `bl31-bl32.bin` file to the root directory of the SD card, because the Raspberry Pi firmware will search for this file to load it in memory.
...
This project was originally developed as part of a student research project.

- João Peixoto - https://github.com/joaopeixoto13
- DRSCODEX - https://github.com/DiogoRoseira